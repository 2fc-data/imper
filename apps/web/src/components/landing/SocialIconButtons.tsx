...
import { INSTAGRAM_URL, WHATSAPP_TEXT, WHATSAPP_URL } from "../../lib/landing";
...
export function WhatsAppIconButton({ className }: { className?: string }) {
  return (
    <a
      ...
      title="WhatsApp"
      className={cn(
        "inline-flex h-8 w-8 items-center justify-center rounded-lg bg-whatsapp text-white transition-colors hover:bg-whatsapp-hover",
        className,
      )}
    >
      <svg
...
export function InstagramIconButton({ className }: { className?: string }) {
  return (
    <a
      ...
      title="Instagram"
      className={cn(
        "inline-flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-tr from-yellow-500 via-pink-500 to-purple-600 text-white transition-transform hover:scale-105",
        className,
      )}
    >
      <svg
